...
public:
  ...
  /* manual serializers
  */
  void serialize_manually(unsigned char* output) {
    unsigned char* tdata=reinterpret_cast<unsigned char*>(_data);
    memcpy(output,tdata,N*sizeof(T));
    // outputstream.write(reinterpret_cast<char*>(_data), N * sizeof(T));
  }
  void deserialize_manually(unsigned char* input) {
    unsigned char* tdata=reinterpret_cast<unsigned char*>(_data);
    memcpy(tdata,input,N*sizeof(T));
