            ...
            mpz_t const &mod_init, mpz_t const &mod_initDiv2)
{
    for (unsigned i = 0; i < Degree; i++) {
        ...
        mpz_set(coefficients[i], coefficients[i]);
        if (mpz_cmp(coefficients[i], mod_initDiv2) > 0)
            mpz_sub(coefficients[i], coefficients[i], mod_init);
//        center(coefficients[i], coefficients[i], mod_init, mod_initDiv2);
        // Multiply by multiplier
        mpz_mul(coefficients[i], coefficients[i], multiplier);
        // Divide by divisor
        mpz_t r;
        mpz_init2(r, mpz_size(divisor) + 1);
        // Compute op1 = rop * op2 + r
        // where r has the same sign as op1
        mpz_tdiv_qr(coefficients[i], r, coefficients[i], divisor);
        // Correct "rop" so that r is centered around 0
        long sgn = mpz_sgn(r);
        mpz_abs(r, r);
        if (mpz_cmp(r, divisorDiv2) >= 0) {
            if (sgn > 0)
                mpz_add_ui(coefficients[i], coefficients[i], 1);
            else
                mpz_sub_ui(coefficients[i], coefficients[i], 1);
        }
        // Clean
        mpz_clear(r);
