...
public:
    ...
    SecretKey(SecKeyStruct sk_bin) :
        value()
    {
        value.deserialize_manually(sk_bin.value_bin);
        value_shoup.deserialize_manually(sk_bin.value_shoup_bin);
    }

    SecKeyStruct output(){
        SecKeyStruct sk_bin;
        value.serialize_manually(sk_bin.value_bin);
        value_shoup.serialize_manually(sk_bin.value_shoup_bin);
        return sk_bin;
    }
...
public:
    ...
    Ciphertext2(CT2Struct ct2_bin):
        c0(),c1()
    {
        c0.deserialize_manually(ct2_bin.c0_bin);
        c1.deserialize_manually(ct2_bin.c1_bin);
    }

    CT2Struct output(){
        CT2Struct ct2_bin;
        c0.serialize_manually(ct2_bin.c0_bin);
        c1.serialize_manually(ct2_bin.c1_bin);
        return ct2_bin;
    }
...
public:
    Ciphertext3(CT3Struct ct3_bin):
        c0(),c1(),c2()
    {
        c0.deserialize_manually(ct3_bin.c0_bin);
        c1.deserialize_manually(ct3_bin.c1_bin);
        c2.deserialize_manually(ct3_bin.c2_bin);
    }

    CT3Struct output(){
        CT3Struct ct3_bin;
        c0.serialize_manually(ct3_bin.c0_bin);
        c1.serialize_manually(ct3_bin.c1_bin);
        c2.serialize_manually(ct3_bin.c2_bin);
        return ct3_bin;
    }
    ...
    // Multiply degree-2 ciphertexts without relinearization.
    Ciphertext3 mul_norelin(const Ciphertext3& ct) const
    {
        using PrmsCT = ParamsCT<T, Degree, NbModuli>;
        using PZ = PolyZ<T, Degree, NbModuli>;
        ...
        return Ciphertext3(std::move(r0), std::move(r1), std::move(r2));
    }    
